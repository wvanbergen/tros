...
module Tros::IPC
  ...
  HANDSHAKE_RESPONSE_SCHEMA = Tros::Schema.parse <<-JSON
  {
    "type": "record",
  ...
  HANDSHAKE_REQUESTOR_WRITER = Tros::IO::DatumWriter.new(HANDSHAKE_REQUEST_SCHEMA)
  HANDSHAKE_REQUESTOR_READER = Tros::IO::DatumReader.new(HANDSHAKE_RESPONSE_SCHEMA)
  HANDSHAKE_RESPONDER_WRITER = Tros::IO::DatumWriter.new(HANDSHAKE_RESPONSE_SCHEMA)
  HANDSHAKE_RESPONDER_READER = Tros::IO::DatumReader.new(HANDSHAKE_REQUEST_SCHEMA)

  META_SCHEMA = Tros::Schema.parse('{"type": "map", "values": "bytes"}')
  META_WRITER = Tros::IO::DatumWriter.new(META_SCHEMA)
  META_READER = Tros::IO::DatumReader.new(META_SCHEMA)

  SYSTEM_ERROR_SCHEMA = Tros::Schema.parse('["string"]')
  ...
  class Requestor
    ...
    def request(message_name, request_datum)
      ...
      buffer_encoder = Tros::IO::BinaryEncoder.new(buffer_writer)
      ...
      buffer_decoder = Tros::IO::BinaryDecoder.new(StringIO.new(call_response))
    ...
    def write_call_request(message_name, request_datum, encoder)
      ...
      #   * the message name, an Tros string, followed by
    ...
    def write_request(request_schema, request_datum, encoder)
      datum_writer = Tros::IO::DatumWriter.new(request_schema)
  ...
  class Responder
    ...
    def respond(call_request, transport=nil)
      buffer_decoder = Tros::IO::BinaryDecoder.new(StringIO.new(call_request))
      buffer_writer = StringIO.new('', 'w+')
      buffer_encoder = Tros::IO::BinaryEncoder.new(buffer_writer)
      error = nil
      response_metadata = {}
    ...
    def process_handshake(decoder, encoder, connection=nil)
      ...
      if !remote_protocol && client_protocol
        remote_protocol = Tros::Protocol.parse(client_protocol)
  ...
  class HTTPTransceiver
    ...
    def transceive(message)
      ...
      resp = @conn.post('/', writer.to_s, {'Content-Type' => 'tros/binary'})
